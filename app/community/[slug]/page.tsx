      ...
      <div style={{maxWidth: '720px', margin: '0 auto', padding: '24px 32px'}}>
        ...
        <div style={{backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '20px 24px', marginBottom: '12px'}}>
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px'}}>
            <div>
              <h1 style={{fontSize: '24px', fontWeight: '600', color: '#1a1a1a', marginBottom: '6px', margin: '0 0 6px 0'}}>{community.canonical_name}</h1>
              <div style={{fontSize: '13px', color: '#888', marginBottom: '12px'}}>{community.street_address}, {community.city}, FL {community.zip_code} · {community.county} County</div>
              <div style={{display: 'flex', gap: '6px', flexWrap: 'wrap'}}>
                <span style={{fontSize: '11px', padding: '3px 8px', borderRadius: '4px', backgroundColor: '#E6F1FB', color: '#0C447C'}}>{community.property_type}</span>
            ...
            <div style={{textAlign: 'right'}}>
              <div style={{fontSize: '12px', color: '#888', marginBottom: '4px'}}>Data confidence</div>
              <div style={{fontSize: '28px', fontWeight: '600', color: '#1D9E75'}}>{community.confidence_score}%</div>
              <div style={{fontSize: '11px', color: '#888'}}>Updated {community.data_freshness_date}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '12px'}}>
          {[
            {val: `$${community.monthly_fee_min}–$${community.monthly_fee_max}/mo`, label: 'Monthly fee', src: 'listing-derived'},
            ...
            <div key={stat.label} style={{backgroundColor: '#f5f5f5', borderRadius: '8px', padding: '12px', textAlign: 'center'}}>
              <div style={{fontSize: '14px', fontWeight: '500', color: '#1a1a1a', marginBottom: '2px'}}>{stat.val}</div>
        ...
        <div style={{backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '20px 24px', marginBottom: '12px'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '14px'}}>
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>${community.monthly_fee_min} – ${community.monthly_fee_max}/mo</div>
              <div style={{fontSize: '11px', color: '#888', marginTop: '2px'}}>Observed range</div>
            </div>
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>${community.monthly_fee_median}/mo</div>
              <div style={{fontSize: '11px', color: '#888', marginTop: '2px'}}>Normalized median</div>
            </div>
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>{community.fee_observation_count} listings</div>
        ...
        {community.assessment_signal_count > 0 && (
          <div style={{backgroundColor: '#fff', border: '1px solid #EF9F27', borderRadius: '12px', padding: '20px 24px', marginBottom: '12px'}}>
            ...
            <div style={{fontSize: '13px', color: '#666', lineHeight: '1.5'}}>Assessment signals have been detected for this community from listing descriptions or user reports. Get the full report for complete details.</div>
          </div>
        )}
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px'}}>
            {[
              ...
              <div key={r.label} style={{display: 'flex', gap: '8px', alignItems: 'flex-start', backgroundColor: '#f9f9f9', borderRadius: '8px', padding: '8px 10px'}}>
                <span style={{fontSize: '13px', color: r.val === 'Prohibited' ? '#E24B4A' : r.val === 'Unknown' ? '#EF9F27' : '#1D9E75', flexShrink: 0}}>
                  {r.val === 'Prohibited' ? '✕' : r.val === 'Unknown' ? '?' : '✓'}
                </span>
                <div>
        ...
        {community.legal_name && (
          <div style={{backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '20px 24px', marginBottom: '12px'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px'}}>
              ...
              {community.state_entity_number && <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: '#888'}}>Entity number</span><span style={{color: '#1a1a1a'}}>{community.state_entity_number}</span></div>}
              {community.incorporation_date && <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: '#888'}}>Incorporated</span><span style={{color: '#1a1a1a'}}>{community.incorporation_date}</span></div>}
              {community.registered_agent && <div style={{display: 'flex', justifyContent: 'space-between'}}><span style={{color: '#888'}}>Registered agent</span><span style={{color: '#1a1a1a'}}>{community.registered_agent}</span></div>}
            </div>
          </div>
        )}
        ...
        <div style={{backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '20px 24px', marginBottom: '12px'}}>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px'}}>
            ...
            <div><strong style={{fontWeight: '500'}}>MLS listing data ({community.fee_observation_count} observations)</strong> <span style={{color: '#888'}}>— Fee range, restrictions, assessment mentions.</span></div>
          ...
          <div style={{marginTop: '12px', fontSize: '12px'}}>
            <a href="#" style={{color: '#1D9E75'}}>Submit a correction or additional source →</a>
          </div>
        </div>
