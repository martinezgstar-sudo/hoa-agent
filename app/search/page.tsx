...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string, city?: string, type?: string }> }) {
  ...
  if (type) {
    dbQuery = dbQuery.ilike('property_type', `%${type}%`)
  }
  ...
  return (
    <main style={{fontFamily:'system-ui,sans-serif',backgroundColor:'#f9f9f9',minHeight:'100vh'}}>
        ...
        <div style={{maxWidth:'720px',margin:'0 auto'}}>
          ...
          <form action="/search" method="GET">
            <div style={{display:'flex',gap:'8px',marginBottom:'12px'}}>
              <input name="q" type="text" defaultValue={query} placeholder="Search by community name, city, zip code, or address..." style={{flex:1,border:'1.5px solid #1a1a1a',borderRadius:'10px',padding:'10px 16px',fontSize:'14px',outline:'none'}}/>
          ...
          <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
            <span style={{fontSize:'11px',color:'#888',paddingTop:'4px'}}>Filter by city:</span>
            {cities.map((c) => (
              <a key={c} href={`/search?${query ? `q=${query}&` : ''}city=${encodeURIComponent(c)}`}
                style={{fontSize:'11px',padding:'3px 10px',borderRadius:'20px',border:'1px solid #e5e5e5',backgroundColor: city === c ? '#1a1a1a' : '#fff',color: city === c ? '#fff' : '#555',textDecoration:'none',cursor:'pointer'}}>
                {c}
              </a>
            ))}
            {city && (
              <a href={`/search${query ? `?q=${query}` : ''}`} style={{fontSize:'11px',padding:'3px 10px',borderRadius:'20px',border:'1px solid #e5e5e5',backgroundColor:'#fff',color:'#E24B4A',textDecoration:'none'}}>
                ✕ Clear filter
              </a>
            )}
          </div>
      ...
      <div style={{maxWidth:'720px',margin:'0 auto',padding:'20px 32px'}}>
        <div style={{fontSize:'12px'olor:'#888',marginBottom:'16px'}}>
        ...
        {communities?.length === 0 && (
          <div style={{textAlign:'center',padding:'60px',color:'#888',fontSize:'14px'}}>
            No communities found. Try a different search or clear your filters.
          </div>
        )}
          ...
          return (
            <a key={c.id} href={`/community/${c.slug}`} style={{textDecoration:'none'}}>
              <div style={{backgroundColor:'#fff',border:'1px solid #e5e5e5',borderRadius:'12px',padding:'16px 20px',marginBottom:'10px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',cursor:'pointer'}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:'15px',fontWeight:'500',color:'#1a1a1a',marginBottom:'3px'}}>{c.canonical_me}</div>
                  <div style={{fontSize:'12px',color:'#888',marginBottom:'8px'}}>
                    {c.city}
                    {c.property_type ? ` · ${c.property_type}` : ''}
                    {c.unit_count ? ` · ${c.unit_count} units` : ''}
                  </div>
                  <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
    ...
    ooter style={{borderTop:'1px solid #e5e5e5',padding:'24px 32px',textAlign:'center',fontSize:'12px',color:'#888'}}>
