...
export default async function CommunityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const amenitiesList = community.amenities ? community.amenities.split(',').map(a => a.trim()) : []
    ...
    <main style={{fontFamily: 'system-ui, sans-serif', margin: 0, padding: 0, backgroundColor: '#f9f9f9'}}>
      ...
      <div style={{maxWidth: '720px', margin: '0 auto', padding: '24px 32px'}}>
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '12px'}}>
          {[
            {
              val: community.monthly_fee_min && community.monthly_fee_max
                ? `$${community.monthly_fee_min}–$${community.monthly_fee_max}/mo`
                : 'Unknown',
              label: 'Monthly fee',
              src: 'listing-derived'
            },
            {
              val: community.review_avg ? `${community.review_avg}★` : 'No reviews',
              label: `${community.review_count || 0} reviews`,
              src: 'user-submitted'
            },
            {
              val: `${community.assessment_signal_count || 0} signals`,
              label: 'Assessments',
              src: 'listing + user'
            },
            {
              val: community.management_company || 'Unknown',
              label: 'Management',
              src: 'public record'
            },
          ].map((stat) => (
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '14px'}}>
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>
                {community.monthly_fee_min && community.monthly_fee_max
                  ? `$${community.monthly_fee_min} – $${community.monthly_fee_max}/mo`
                  : '—'}
              </div>
            ...
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>
                {community.monthly_fee_median ? `$${community.monthly_fee_median}/mo` : '—'}
              </div>
              <div style={{fontSize: '11px', color: '#888', marginTop: '2px'}}>Normalized median</div>
            </div>
            <div>
              <div style={{fontSize: '16px', fontWeight: '500', color: '#1a1a1a'}}>
                {community.fee_observation_count || '—'}
                {community.fee_observation_count ? ' listings' : ''}
              </div>
